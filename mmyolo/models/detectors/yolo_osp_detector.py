...
class YOLOOSPDetector(SingleStageDetector):
    ...
    def __init__(self,
        ...
        self.flip_enhancement = flip_enhancement
        # init visualizer
        ...
        self.visualizer.dataset_meta = {
            ...
            'palette': [(101, 205, 228), (240, 128, 128), (255, 0, 255), (0, 139, 139), (255, 165, 0)]
        }
        ...
        self.dir_result_display = f'{current_directory}/displayed_results/OSP-yolo/{dataset_name}/{current_time}/'
    ...
    def predict(self,
        ...
        #     cv2.imwrite(os.path.join(dir_file, f'Reshaped_ori_img.png'), ori_img)
        ...
        #         #######################################################
        #         num_classes = len(self.bbox_head.head_module.cls_preds[level].out_conv.weight) - 1
        #         # 获取所有 GT 类别索引（ind_class[batch]）并添加背景类别索引（num_classes）
        #         cls_indices = torch.cat([
        #             ind_class[batch],
        #             torch.tensor([num_classes], dtype=ind_class[batch].dtype, device=ind_class[batch].device)
        #         ])  # 假设背景类别索引为 num_classes
        #         cls_prototypes = self.bbox_head.head_module.cls_preds[level].out_conv.weight[cls_indices]
        ...
        #             superimposed_img = cv2.addWeighted(heatmap, 0.4, ori_img, 0.6, 0)
        #             #######################################################
        #             cls_id = cls_indices[k].item()
        #             cls_name = INDEX_TO_CLS[self.dataset_name].get(
        #                 cls_id, f"Background_{cls_id}" if cls_id == num_classes else f"Unknown_{cls_id}")
        ...
        #         device = instance_vectors[0].device
        #         strides = [8, 16, 32]
        #         stride = strides[level]
        #         reg_min = [0, 0, 0]
        #         reg_max = [4, 8, 16]
        #         # 获取当前层的预测范围
        #         min_size = reg_min[level] * stride * 2  # 最小预测尺寸
        #         max_size = reg_max[level] * stride * 2  # 最大预测尺寸
        #         h, w = instance_vectors[level].shape[2:]
        #         # Initialize target and prediction saliency maps
        #         target_saliency = torch.zeros((num_classes + 1, h, w), device=device)
        ...
        #                 target_saliency[cls_idx] = torch.logical_or(
        #                     target_saliency[cls_idx], positive_points).float()
        #                 # 更新全局前景掩码
        #                 fg_mask = torch.logical_or(fg_mask, positive_points)
        #         # 处理背景区域 (非前景区域即为背景)
        #         bg_mask = ~fg_mask
        #         target_saliency[num_classes] = bg_mask.float()
        #         # 保存 target_saliency 每个类别的图像
        #         for cls_idx in range(num_classes + 1):  # 包含背景
        #             saliency_map = target_saliency[cls_idx].detach().cpu().numpy()
        #             saliency_uint8 = 255 * saliency_map.astype(np.uint8)
        #
        #             # 设置保存名称
        #             if cls_idx == num_classes:
        #                 cls_name = "Background"
        #             else:
        #                 cls_name = INDEX_TO_CLS[self.dataset_name].get(cls_idx, f"Unknown_{cls_idx}")
        #
        #             save_path = os.path.join(dir_file, f"target_saliency-layer{level + 1}--class_{cls_name}.png")
        #             cv2.imwrite(save_path, saliency_uint8)
        #
        #
        ...
        #         draw_pred=True, pred_score_thr=0.5,
